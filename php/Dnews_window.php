    ...
    <tr>
      <td>
		...
		//execute an SQL statement and return a recordset
          $query  = "SELECT t1.oname,t1.Tweet,t1.TTime,t2.Likes,t1.Name FROM Congress.Tweets2 t1, Congress.FB t2 WHERE t1.Party = 'D' AND t1.Code = t2.Code order by t1.GTime DESC";
...
<table cellpadding='5' cellspacing='0' border='0' width='100%' style='background-color: rgb(255, 255, 255); color: rgb(0, 0, 0); font: normal normal normal 11px/normal arial, sans-serif; font-weight: normal; '>
	<tr>
		...
		//execute an SQL statement and return a recordset
          $query  = "SELECT t1.oname,t1.Tweet,t1.TTime,t2.Debt,t1.Name FROM Congress.Tweets2 t1, Congress.HouseMembers t2 WHERE t1.Party = 'D' AND t1.Code = t2.Code order by t1.GTime DESC";
          $result = mysql_query($query)or die(mysql_error());
          while($row = mysql_fetch_assoc($result)) {
           $Name      = $row['oname'];
           $Tweet     = $row['Tweet'];
...
<table cellpadding='5' cellspacing='0' border='0' width='100%' style='background-color: rgb(255, 255, 255); color: rgb(0, 0, 0); font: normal normal normal 11px/normal arial, sans-serif; font-weight: normal; '>
	<tr>
       ...
       if($day == Wed){
	    ...
	    //execute an SQL statement and return a recordset
        $query  = "SELECT t1.oname,t1.Tweet,t1.TTime,t2.CashOnHand,t1.Name FROM Congress.Tweets2 t1, Congress.HouseMembers t2 WHERE t1.Party = 'D' AND t1.Code = t2.Code order by t1.GTime DESC";
        $result = mysql_query($query)or die(mysql_error());
        while($row = mysql_fetch_assoc($result)) {
         $Name      = $row['oname'];
         $Tweet     = $row['Tweet'];
...
<table cellpadding='5' cellspacing='0' border='0' width='100%' style='background-color: rgb(255, 255, 255); color: rgb(0, 0, 0); font: normal normal normal 11px/normal arial, sans-serif; font-weight: normal; '>
	<tr>
		...
		<table id='contents' border='0' cellpadding='3' cellspacing='2' width='95%' height='68' align='left' bordercolor='#C40000' style='background-color: rgb(255, 255, 255); color: rgb(0, 0, 0); font: normal normal normal 11px/normal arial, sans-serif; font-weight: normal; border-collapse: collapse; '>
		 ...
		 //execute an SQL statement and return a recordset
      $query  = "SELECT oname,Tweet,TTime,Foll,Name FROM Congress.Tweets2 WHERE Party = 'D' order by GTime DESC";
      $result = mysql_query($query)or die(mysql_error());
      while($row = mysql_fetch_assoc($result)) {
       $Name      = $row['oname'];
       $Tweet     = $row['Tweet'];
